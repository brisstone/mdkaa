// this was created to fix cyclic redundancy btw VisitorController

// using 1. UserController.getCompany
...
// the above require was used in NotifyIpadAdmin, commented also


// console.log("mmmmmmmmmmmmmm",visitorsuitEvents)
...
var UserControllerClone = module.exports = {
  ...
  async addStaff(req, res, staff) {
    try {
      for (let i = 0; i < staff.length; i += 1) {
        ...
        if (!visitorSuite) {
          const visitorsuite = await visitorsuite.create({
            id: null,
            company: req.user.company,
            workspace_company: req.user.workspace_company,
            email: data.email,
          ...
          if (visitorsuite) {
            const payload = {
              staff: visitorsuite.id,
              company: visitorsuite.company
            };
    ...
    try {
      if (req.user.role === 'GLOBAL_ADMIN' || req.user.role === 'LOCATION_ADMIN') {
        ...
        if (staff) {
          ...
          visitorsuitEvents.emit('sendAdminInvite', globalAdmin, staff, role, url, location);
  ...
  async getStaff7daysAttendance(req, res) {
    ...
    if (
      ...
      try {
        ...
        if (staff) {
          const data = await models.staff_attendance.findAndCount({
            where: {
              staff: id,
              date: {
                $between: [date, toDate]
...
module.exports.UserControllerClone =UserControllerClone
